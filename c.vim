...
syn keyword     cSection        contained SECTION
...
syn cluster	cCommentGroup	contains=cTodo,cBadContinuatio,cNote,cSection
...
syn match	cJCParamType	"\<\(\(const\|restrict\|volatile\|signed\|unsigned\|struct\|enum\|union\)[ \t*]\+\)*\I\i*[ \t*]\+\I"he=e-1 contained containedin=cJCFor

" Matchs for types in variables and function declarations
syn match	cJCTypeInDecl	"^\s*\(\(inline\|const\|restrict\|extern\|GLOBAL\|static\|register\|auto\|volatile\|virtual\|signed\|unsigned\|struct\|union\)[ \t*]\+\)*\I\i*\([ \t*]\+\(const\|restrict\|volatile\)\)*[ \t*]*" contained
syn match	cJCDecl		"^\s*\(inline\s\+\)\=\(\I\i*[ \t*]\+\)\+\s*\I" contains=cJCTypeInDecl

" Matches function declarations and definitions
syn region	cJCFunc		start="^\(\(inline\|const\|extern\|GLOBAL\|static\|register\|auto\|volatile\|virtual\|signed\|unsigned\|struct\|union\|float\)[ \t*]\+\)*\I\i*\s\+\**\s*\I[[:ident:]:]*\s*(" end=")" contains=CJCParamVoid,cJCParamType,cJCTypeInDecl,cComment
syn region	cJCFunc		start="^\I\i*\s*(" end=")" contains=CJCParamVoid,cJCParamType

" Matches type casts
syn match	cJCTypeCast	"(\@<=\s*\(\(const\|restrict\|volatile\|signed\|unsigned\|struct\|enum\|union\)[ \t*]\+\)*\I\i*\s*\**\s*\(restrict\)\?\s*)\s*[^) \t;,{]"me=e-2
...
"hi cJCTypeCast	ctermbg=#ff0000
"hi cJCDecl		ctermbg=#d08080
"hi cJCType		ctermbg=#FFFF00
"hi cJCTypeInDecl	ctermbg=#FF00FF
"hi cJCFunc		ctermbg=#00FF00
"hi cJCParamType	ctermbg=#0000FF
"hi cJCIf		ctermbg=#00FFFF
"hi cJCFor		ctermbg=#00FF80
